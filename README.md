...
`MISSING_DATA_TOLERANCE` (default `120` seconds) is how long this is waited out. A restart of a single battery driver takes roughly 50 seconds, so the default carries real margin over the case it exists for. It deliberately does not cover a compound restart, one landing on another that is still initialising, which was measured at 125 to 134 seconds of absence: a battery missing for two minutes is a problem in its own right, and escalating it is a better answer than sitting it out silently. Both figures come from one two-battery BLE system on a Cerbo GX. When the values are complete again, publishing resumes and the log says how long the gap lasted. When the tolerance is exceeded, the read is counted as a failure as before and the driver exits after `READ_TRIALS` so that the service manager restarts it. That log line names the battery and how long it has been missing, so a system that genuinely needs a longer window says which number it needs. Set `MISSING_DATA_TOLERANCE = 0` to switch the waiting off and count read failures immediately.

The published values must therefore be treated as potentially up to `MISSING_DATA_TOLERANCE` seconds old. Nothing on the DBus marks them as stale: while waiting, the last complete aggregate stays exactly as it was, and consumers, DVCC included, act on it. On a bank at rest that costs nothing; under load the published current is that much older than the bank it describes, and a battery that is genuinely broken or has been removed is tolerated in silence for as long as the window lasts. That is why the default is sized to the ordinary case with margin rather than to the worst case seen: every second added to the window is a second of acting on values the aggregation already knows are stale.
